...
ApproxForm = on; % Enables approximate formula for uncertainty calculation
...
%BS_iterations = 1000; %Number of bootstrapping iterations
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
b_combined = []; % Initialize

%solved_param_MC_BS = zeros(N_MC * N_BS, 12); 
...
%         Par2_label = par_names(Ifitpar(2),1) + ' [' + par_names(Ifitpar(2),2) + ']';
        ...
        if ApproxForm == 1
            ...
            % Compute residuals and MSEC
            residuals = dTemp-Sfit;
            N = numel(dTemp);          % Number of data points
            p = numel(fitresult);       % Number of parameters
            MSEC = sum(residuals.^2) / (N - p); % Residual variance

            % Compute numerical Jacobian
            epsilon = 1e-6; % Small perturbation
            num_params = numel(fitresult);
            num_points = numel(Time);
            X = zeros(num_points, num_params);
            
            b_pert_param = param;

            for j = 1:num_params
                % Perturb parameter j
                b_perturbed = fitresult;
                b_perturbed(j) = b_perturbed(j) + epsilon;
                
                b_pert_param(Ifitpar)=b_perturbed;

                % Evaluate model with perturbed parameter
                y_perturbed = NeedleProbeModel(Time, b_pert_param, cp,IV);

                % Numerical derivative for parameter j
                X(:, j) = (y_perturbed - Sfit) / epsilon;
            end

            % Calculate covariance matrix of parameters using backslash operator
            cov_b = MSEC * pinv(X' * X);

            % Extract parameter uncertainties
            param_std = (sqrt(diag(cov_b)))'; % Standard deviation (uncertainty) for each parameter
            ...
            disp(fitresult);
            disp('Parameter Uncertainties:');
            disp(param_std);
            
            % Define the file to store bootstrap results
            filenm = 'ApproxFormula.txt';
            
            % Open the file in append mode
            file = fopen([run_name, filenm],'at'); %fopen('\Analysis_Results\',' MC_runinfo.txt');
                       
            fprintf(file, '%s\t', num2str(run));
            fprintf(file, '%s\t', num2str(aveTemp));
            fprintf(file, '%.7e\t', MSEC); % Write each row with space-separated values
            fprintf(file, '%.7e\t', fitresult); 
            fprintf(file, '%.7e\t', param_std); 
