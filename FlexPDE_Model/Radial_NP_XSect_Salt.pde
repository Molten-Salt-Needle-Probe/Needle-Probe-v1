...
vol_wires = pi*r_wires^2*(h_max*1)+pi*r_wires^2*(pi*(r_wir_o+r_wir_i)/2)       		  	! Calculating Volume of heating wires
...
top_flux = SURF_INTEGRAL((-k*dz(temp)), 'Top Surface')
bottom_flux = SURF_INTEGRAL((k*dz(temp)), 'Bottom Surface')
cylindrical_flux = SURF_INTEGRAL((-k*dr(temp)), 'Cylindrical Surface')

top_flux_all = SURF_INTEGRAL((-k*dz(temp)), 'Top Surface All')
bottom_flux_all = SURF_INTEGRAL((k*dz(temp)), 'Bottom Surface All')
cylindrical_flux_all = SURF_INTEGRAL((-k*dr(temp)), 'Cylindrical Surface All')

AreaTopBottom = SURF_INTEGRAL(1, 'Top Surface')
AreaTopBottomAll = SURF_INTEGRAL(1, 'Top Surface All')
AreaCylinder = SURF_INTEGRAL(1, 'Cylindrical Surface')
AreaCylinderAll = SURF_INTEGRAL(1, 'Cylindrical Surface All')
AreaTotal = SURF_INTEGRAL(1, 6)


total_flux = SURF_INTEGRAL(normal(-k*grad(temp)),6)
...
MATERIALS!          Thermal Cond.    Density             Specific Heat            Q-Generation if applicable
    ...
    "Sample" : 				k=0.78 		rho=2034.5 			cp=1904.2			! FLiNaK 600C 
    "Atmosphere":			k=0.0398 	rho=0.5525				cp=536			! Argon  600 C
...
! Box enclosing heater portion
 ...
 FEATURE "Cylindrical Surface"
  	START(r_wir_o, h_max)  LINE to (r_wir_o,  r_Ni - r_Al + 0.001)
...
PLOTS
    ...
    vector(-K*dr(Temp), -K*dz(temp))  zoom(0.0, 0.0, .003, h_cruc) as "Heat Flux Total Probe" export format "#t#r,#i" file="TopFlux.txt"  
...
HISTORIES
   ...
   history(cylindrical_flux) as "Cylindrical Heat Rate (W)" export format "#t#r,#i" file="FLiNaK_CylindricalFlux.txt"  
   history((cylindrical_flux_all/AreaCylinderAll)/((top_flux_all+bottom_flux_all)/AreaTopBottomAll)) as "Ratio Radial to Axial Flux (W/mK)" export format "#t#r,#i" file="FLiNaK_Radial_to_Axial_Flux.txt"  
