function [uniqueID,filename] = RunFlexPDE_Axial(par_vector,par_names,SolvNam,endtime)


%%%%%%%% Prepare geometry of model %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

r_tc = 0.094313e-3;  			    % Radius of Thermocouple
TC_loc = 0%0.05;						% Location of TC Bead w relation to probe tip (5 cm)
...
fprintf(fileID, 'COORDINATES cartesian2\n\n');
...
fprintf(fileID, '"Heating_wires" : \tk=%8.4f \trho=%8.4f \tcp=%8.4f \t\tq_gen=%8.4f\n', k_wire, rho_wire, cp_wire, q_gen_wire);
fprintf(fileID, '"Thermocouple" : \tk=%8.4f \trho=%8.4f \tcp=%8.4f\n\n', k_Thermocouple, rho_Thermocouple, cp_Thermocouple);

% Set initial temperature
fprintf(fileID, '{ Set initial temp: T_amb in definitions }\n');
fprintf(fileID, 'INITIAL VALUES\n');
fprintf(fileID, '\ttemp = T_amb\n\n');

% Write the heat equation with source
fprintf(fileID, '{ Heat Eqt with a Source (q_gen) }\n');
fprintf(fileID, 'EQUATIONS\n');
fprintf(fileID, 'temp:\tdiv(k*grad(temp)) + q_gen = (rho*cp)*dt(temp)\n\n');

% Defining the geometry
fprintf(fileID, '{Defining the geometry}\n');
fprintf(fileID, 'BOUNDARIES\n\n');

% Crucible Region
fprintf(fileID, '  { Crucible Region }\n');
fprintf(fileID, '  REGION 1\n');
fprintf(fileID, '  USE MATERIAL "Crucible"\n');
fprintf(fileID, '    START (r_cruc,0) \n');
fprintf(fileID, '    natural(temp) = h_conv*(T_amb - temp)   { Set flux to be the diffusion/convection equation }\n');
fprintf(fileID, '    ARC (center = 0,0) ANGLE=360 TO CLOSE\n\n');

% Sample Region
fprintf(fileID, '  { Sample Region }\n');
fprintf(fileID, '  REGION 2\n');
fprintf(fileID, '  USE MATERIAL "Sample"\n');
fprintf(fileID, '    START(0,r_samp)\n');
fprintf(fileID, '    ARC (center = 0,0) ANGLE=360 TO CLOSE\n\n');

% Sheath Region
fprintf(fileID, '  { Sheath Region }\n');
fprintf(fileID, '  REGION 3\n');
fprintf(fileID, '  USE MATERIAL "Sheath"\n');
fprintf(fileID, '    START(0,r_Ni) \n');
fprintf(fileID, '    natural(temp) = q_rad\n');
fprintf(fileID, '    ARC (center = 0,0) ANGLE=360 TO CLOSE\n\n');

% Alumina Region
fprintf(fileID, '  { Alumina Region }\n');
fprintf(fileID, '  REGION 4\n');
fprintf(fileID, '  USE MATERIAL "Alumina"\n');
fprintf(fileID, '    START(0,r_Al)\n');
fprintf(fileID, '    Contact(temp) = (1/rTh_alumina_sheath) * JUMP(temp)\n');
fprintf(fileID, '    ARC (center = 0,0) ANGLE=360 TO CLOSE\n\n');

% Heating Wire Region 1 (right)
fprintf(fileID, '  { Heating Wire Region 1 (right) }\n');
fprintf(fileID, '  REGION 5\n');
fprintf(fileID, '  USE MATERIAL "Heating_wires"\n');
fprintf(fileID, '    START(r_wir_i + r_wires, r_wires) \n');
fprintf(fileID, '    Contact(temp) = (1/rTh_wires_alumina) * JUMP(temp)\n');
fprintf(fileID, '    ARC (center = (r_wir_i + r_wires),0) ANGLE = 360 TO CLOSE\n\n');

% Heating Wire Region 2 (left)
fprintf(fileID, '  { Heating Wire Region 2 (left) }\n');
...
% TC Region
fprintf(fileID, '  { TC Region }\n');
fprintf(fileID, '  REGION 7\n');
fprintf(fileID, '  USE MATERIAL "Thermocouple"\n');
fprintf(fileID, '    START(r_tc,0) \n');
fprintf(fileID, '    Contact(temp) = (1/rTh_wires_alumina) * JUMP(temp)  ! Should change for alumel\n');
fprintf(fileID, '    ARC (center = 0,0) ANGLE=360 TO CLOSE\n');
...
fprintf(fileID, 'History(Temp) AT (0.0, 0.0) export format "#t#r,#i" file="temp.txt"\n\n');
