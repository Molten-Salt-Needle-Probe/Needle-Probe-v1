...
foldernames = {'0.5np-NaNO3-KNO3 1-13-25','1np-NaNO3-KNO3 12-5-24 (fumehood)','1np-NaNO3-KNO3 12-18-24 (fumehood batch 2)','NaNO3-KNO3 11-22 (fumehood)','0.5np-NaNO3-KNO3 12-19-24'};
samp_array = {'0_5npNaNO3-KNO3-T2','1npNaNO3-KNO3-T1','1npNaNO3-KNO3-T2','NaNO3-KNO3', '0_5npNaNO3-KNO3-T1'};

for w = 1:length(samp_array)
    cd 'C:\Users\ME\Documents\Peter_Kasper\FlexPDE_Runs\TylersTests\MATLAB Files\MatlabTransfer';
    %clc
    clearvars -except foldernames samp_array w
    close all
    ...
    timewindow = [0.5 50]; % [1 50]; % (15 s) Sets the time interval to be analyzed, in seconds. Set beginning to 0 to start from the beginning
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if global_fitting == 0
        run_name = [char(sample) ' ' joinedString ' ' today '_fminsearch'];
    else
        run_name = [char(sample) ' ' joinedString ' ' today '_global'];
    end
    ...
    [plotfolder, datafolderUSE, aveTemp_vector] = IterExtractData(foldernames(w),run_name,raw_plot,runfolder,timewindow);
    ...
    %         Par2_label = par_names(Ifitpar(2),1) + ' [' + par_names(Ifitpar(2),2) + ']';
        ...
        if Full_Analysis == 1
            ...
            dims = size(fitresult);
            dims_twostd = size(two_std);
            if dims(1)==1 && dims(2)==1
                fitresult = [fitresult,0];
            end
            if dims_twostd(1)==1 && dims_twostd(2)==1
                two_std = [two_std,0];
            end
